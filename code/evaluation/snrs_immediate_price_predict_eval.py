...
# 1. User Preference
#    a. How many times the user visited location of rating k
#    b. How many times the user visited the location of price range say p
...
# Function to get the probability distribution of the price range for all the business the user has visited
def getPriceAttribute(result):
    i = 0
    while i < len(priceAcceptance):
        priceAcceptance[i][0] = 4
        priceAcceptance[i][1] = 1
        priceAcceptance[i][2] = 1
        priceAcceptance[i][3] = 1
        priceAcceptance[i][4] = 1
        i+=1
    i = 0
    size = len(result)
    while i < size:
        try:
            if result[i].business_id.__contains__("-"):
                q = getQuery("business_id", result[i].business_id.lower())
            else:
                q = TermQuery("business_id", result[i].business_id.lower())

            priceResult = conn.search(query=q, size=10000, indices=index_name, type=doc_typeB)

            priceAcceptance[result[i].stars][priceResult[0].attributes["Price Range"]] += 1
            priceAcceptance[result[i].stars][0] += 1
            priceAcceptance[0][0] += 1
            priceAcceptance[0][priceResult[0].attributes["Price Range"]] += 1
            # print result[i].stars
            # print priceResult[0].attributes["Price Range"]
        except:
            print "",
            # continue
        i += 1
...
userAcceptance = [0] * 6
priceAcceptance = [[0 for p1 in range(5)] for p2 in range (6)]
...
# Get the probability distribution for price range
print "Generating the price acceptance matrix"
getPriceAttribute(result)
print priceAcceptance
...
toRecommend = findImmediateFriends(friendList,inputUserBList,userRating_dict,userBusiness_dict,2)
...
# Fetch the price accepance dictionary
f = open('B2P_Dictionary.txt','r')
print "Fetching B2P_Dictionary.txt"
p = pickle.load(f)
f.close()
...
for r in toRecommend:
    ...
    while k < 6:
        if r in p:
            x = float((priceAcceptance[k][p[r]])) * (float(userAcceptance[k])/float(userAcceptance[0])) / float((priceAcceptance[0][p[r]]))
        else:
